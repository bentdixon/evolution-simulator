...
# Statistics
avg_speed = ti.field(dtype=ti.f32, shape=())
avg_size = ti.field(dtype=ti.f32, shape=())
avg_vision = ti.field(dtype=ti.f32, shape=())
...
def update_creatures(dt: ti.f32):
    ...
    for i in range(MAX_CREATURES):
        if creatures[i].alive == 1:
            ...
            if creatures[i].energy <= 0 or creatures[i].age > 100:
                creatures[i].alive = 0

    # Update statistics
    if total_alive[None] > 0:
        avg_speed[None] = sum_speed / total_alive[None]
        avg_size[None] = sum_size / total_alive[None]
        avg_vision[None] = sum_vision / total_alive[None]
...
def main():
    window = ti.ui.Window("Evolution Simulator", (WIDTH, HEIGHT))
    ...
    print("Evolution Simulator Started!")
    print("Controls: ESC to exit")
    print("\nWatch as creatures evolve over time:")
    print("- Green dots: Food")
    print("- Colored circles: Creatures")
    print("- Faint circles: Vision range")
    print("\nCreatures with better traits survive longer!")
    print("\nWandering behavior: Creatures move in a direction for 10-30 steps when no food is visible")
    ...
    while window.running:
        ...
        if frame % 60 == 0:
            print(f"\nGeneration: {max_generation[None]} | Alive: {total_alive[None]}")
            print(f"Avg Speed: {avg_speed[None]:.2f} | Size: {avg_size[None]:.2f} | Vision: {avg_vision[None]:.2f}")
